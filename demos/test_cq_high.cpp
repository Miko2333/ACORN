...
int main(int argc, char *argv[]) {
    ...
    std::cout << "====================\nSTART: running TEST_CQ for hnsw, sift data --" << nthreads << "cores\n" << std::endl;
    ...
    index_file << "./indexes/CQ_" << dataset << "_high.fvecs";
    ...
    // typedef faiss::Node Node;
	// std::vector<Node> hash(nq);
	// const size_t mod1 = 1e9 + 7, mod2 = 1e9 + 9;
    // auto add_mod = [](size_t a, size_t b, size_t mod) {
    //     size_t c = a + b;
    //     if (c >= mod)
    //         c -= mod;
    //     return c;
    ...
    //     hash[i].id = i;
	// 	for (int j = 0; j < N; j++) {
    //         hash[i].x = add_mod(add_mod(hash[i].x, hash[i].x, mod1), filter_ids_map[i * N + j], mod1);
    //         hash[i].y = add_mod(add_mod(hash[i].y, hash[i].y, mod2), filter_ids_map[i * N + j], mod2);
    //     }
	// }
    ...
    std::cout << "============= CQ QUERY PROFILING STATS =============" << std::endl;
